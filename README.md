...
音楽編集ソフトDAWのプラグインとスタンドアロンの両立、
最大20人がインターネット経由でリモート音楽セッションを行えることを目標としている。
## 概要

SYNCROOM（ヤマハ）や SonoBus のような
超低遅延音声通話アプリを個人で一から実装したプロジェクトです。

### SYNCROOM・SonoBus とは？

**SYNCROOM**（ヤマハ）
インターネット経由でミュージシャン同士が
リモートで合奏できるサービス。
ZoomやLINEでは遅延が大きすぎて
音楽セッションが困難なため、
遅延を極限まで抑えた専用技術を使っている。
同時接続は最大6人。

**SonoBus**
無料・オープンソースの低遅延音声通信アプリ。
インターネット経由で複数人がリアルタイムに
音声を共有できる。
スタンドアロンアプリとDAWプラグインの
両方に対応している。

### このプロジェクトとの違い

| 項目 | SYNCROOM | SonoBus | このプロジェクト |
|---|---|---|---|
| 同時接続数 | 最大6人 | 制限なし | **最大20人** |
| 接続方式 | P2P | P2P | **専用サーバー** |
| DAW連携 | VST対応 | VST/AAX対応 | **VST3+パラ出力** |
| ライセンス | 無料 | 無料（OSS） | MIT（OSS） |
| 対象 | 一般公開 | 一般公開 | **プライベート用途** |
